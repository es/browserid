...
exports.setup = function(app) {
  ...
  var persona = fs.readFileSync(path.join(__dirname, "..", "..", "resources", "static", "robots-persona.txt")).toString();
  var anosrep = fs.readFileSync(path.join(__dirname, "..", "..", "resources", "static", "robots-anosrep.txt")).toString();
  app.get("/robots.txt", function(req, res) {
    cacheAllTheThings(res);
    res.setHeader('Content-Type', 'text/plain; charset=utf8');
    if (req.header('host').indexOf('anosrep.org') !== -1 || config.get('env') !== 'production') {
      res.send(anosrep);
    }
    else {
      res.send(persona);
